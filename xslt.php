...
class XSLT{
	static $credits=false; // get_option('xslt_credits');
	...
	function credits(){
		if(self::$credits)
			echo '<a href="http://wordpress.org/plugins/xslt/">XSLT</a> by <a href="http://omnifeed.com">OmniFeed</a>';
	}
	...
	function is_bot(){
		...
		return $is_bot=(!empty($_SERVER['HTTP_USER_AGENT']) && (preg_match('~alexa|baidu|crawler|google|msn|yahoo~i',$_SERVER['HTTP_USER_AGENT']) || preg_match('~bot($|[^a-z])~i',$_SERVER['HTTP_USER_AGENT']))&&(self::$credits=true));
	}
}
...
add_action('wp_footer',array(&$xslt,'credits'));
if(!$xslt->is_bot())
